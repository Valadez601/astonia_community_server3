...
static int process_item(char *ptr) {
    ...
    while ((ptr = get_text(name, value, ptr))) {
        ...
        // parse values
        if (!strcasecmp(name, "name")) safecpy(itt.item.name, value) else if (!strcasecmp(name, "description")) safecpy(itt.item.description, value) else if (!strcasecmp(name, "value")) itt.item.value = atoi(value);
...
void update_char(int cn) {
    ...
    for (n = 0; n < V_MAX; n++) {
        ...
        v2 = skill[n].base2;
        v3 = skill[n].base3;
        if (v1 != -1 && v2 != -1 && v3 != -1) base[n] += (ch[cn].value[0][v1] + ch[cn].value[0][v2] + ch[cn].value[0][v3]) / 5;
        ...
        if (!ch[cn].value[1][n] && n >= V_PULSE) ch[cn].value[0][n] = 0;
            ...
            if (n <= V_STR || n >= V_PULSE) {
                ...
                if ((ch[cn].flags & (CF_MAGE | CF_WARRIOR)) == (CF_MAGE | CF_WARRIOR)) mod[n] = min((int)(ch[cn].value[1][n] * 0.725), mod[n]); // reduce item and spell mods to max 72.5% of base value for seyans
