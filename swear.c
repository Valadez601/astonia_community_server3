...
int swearing(int cn, char *text) {
    ...
    if (ppd->banned_till > realtime) {
        log_char(cn, LOG_SYSTEM, 0, "\260c3Chat is blocked for %.2f minutes.", (ppd->banned_till - realtime) / 60.0);
        return 1;
    }
    ...
    if (strcasestr(text, "fuck") || strcasestr(text, "cunt") || strcasestr(text, "faggot")) {
        log_char(cn, LOG_SYSTEM, 0, "\260c3Swearing is illegal in this game. While only a few words are blocked by the system, you will get punished and eventually banned if you swear using non-blocked words.");
        log_char(cn, LOG_SYSTEM, 0, "\260c3Chat has been blocked for 30 seconds.");
        ppd->bad = realtime;
        return 1;
    }

    if (strlen(text) > 3 && all_upper(text)) {
        log_char(cn, LOG_SYSTEM, 0, "\260c3Using capitalized letters only is impolite. Trying to get around the block by using mostly caps will get you punished and eventually banned.");
        log_char(cn, LOG_SYSTEM, 0, "\260c3Chat has been blocked for 30 seconds.");
        ppd->bad = realtime;
        return 1;
    }
        ...
        for (n = flag = 0; n < 10; n++) {
            if (!strncmp(ppd->last_sentence[n], text, 78) && realtime - ppd->last_time[n] < 30) {
                if (ppd->last_cnt[n] > 2 || realtime - ppd->last_time[n] < 4) {
                    log_char(cn, LOG_SYSTEM, 0, "\260c3Repeating the same sentence is impolite. Repeating variants of the same sentence will get you punished and eventually banned.");
                    log_char(cn, LOG_SYSTEM, 0, "\260c3Chat has been blocked for 30 seconds.");
                    ppd->bad = realtime;
                    return 1;
                }
...
void shutup_bg(int cnID, int coID, int minutes) {
    ...
    if (minutes > 0) log_char(co, LOG_SYSTEM, 0, "\260c3Your ability to talk has been disabled.");
    else log_char(co, LOG_SYSTEM, 0, "\260c3Your ability to talk has been enabled.");
