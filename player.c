...
static void read_login(int nr) {
    ...
    prof = prof_start(29);
    ret = find_login(name, password, &area, &cn, &mirror, &ID, vendor, &unique, htonl(player[nr]->addr));
    prof_stop(29, prof);
    ...
    if (ret == -8) {
        player_client_exit(nr, "Please log onto your account management on www.astonia.com and update the account ownership information. Scroll down to 'Address Information' and choose 'Edit'.");
        return;
    }
    if (ret == -9) {
