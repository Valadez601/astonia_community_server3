...
void score_fight(int cn, int co, struct master_data *dat) {
    ...
    p1 = set_data(cn, DRD_ARENA_PPD, sizeof(struct arena_ppd));
    if (!p1->fights) p1->score = -2000;
    p2 = set_data(co, DRD_ARENA_PPD, sizeof(struct arena_ppd));
...
void check_fight(int cn, struct master_data *dat) {
    ...
    if (ch[dat->fight1_cn].x <= 233 || ch[dat->fight1_cn].x >= 243 || ch[dat->fight1_cn].y <= 132 || ch[dat->fight1_cn].y >= 142) {
        end++;
        win1 = 0;
    }
    if (ch[dat->fight2_cn].x <= 233 || ch[dat->fight2_cn].x >= 243 || ch[dat->fight2_cn].y <= 132 || ch[dat->fight2_cn].y >= 142) {
        end++;
        win2 = 0;
    }
...
void master_driver(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            /*case 2:		ppd=set_data(co,DRD_AREA3_PPD,sizeof(struct area3_ppd));
                        ...
                        break;*/
            case 3: // register
                ppd = set_data(co, DRD_ARENA_PPD, sizeof(struct arena_ppd));
    ...
    case 0:
        if (create_storage(dat->storage_ID, "Arena Toplist Data", &dat->storage_data, sizeof(dat->storage_data))) dat->storage_state++;
        break;
    case 1:
...
void fighter_driver(int cn, int ret, int lastact) {
    ...
    if (dat->storage_state > 3) {
        switch (dat->state) {
        case FS_LEISURE:
            if ((abs(ch[cn].x - ch[cn].tmpx) > 2 || abs(ch[cn].y - ch[cn].tmpy) > 2) && move_driver(cn, ch[cn].tmpx, ch[cn].tmpy, 2)) return;
            if (ticker - dat->lastact < TICKS * 60 * 3) break;
            dat->state++;
            break;
        case FS_START:
            if (abs(ch[cn].x - MASTER_POSX) < 5 && abs(ch[cn].y - MASTER_POSY) < 5) dat->state++;
    ...
    switch (dat->storage_state) {
    case 0:
        if (create_storage(dat->storage_ID, "Arena Fighter Data", &dat->storage_data, sizeof(dat->storage_data))) dat->storage_state++;
        break;
    case 1:
        if (check_create_storage()) dat->storage_state++;
        break;
