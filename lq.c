...
void cmd_exit(int cn, char *ptr) {
    ...
    if ((pdat = set_data(cn, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data)))) {
        co = pdat->usurp;
        pdat->usurp = 0;
    }
...
void cmd_questshow(int cn, char *ptr) {
    ...
    for (n = 1; n < MAXLQMARK; n++) {
        if (lq_data.reward[n]) log_sys(cn, "Reward for mark %s (%d) is %d exp.", lq_data.reward_desc[n], n, lq_data.reward[n]);
    }
}
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_GOTHIT) {
            ...
            if (co && (ch[co].flags & CF_PLAYER) &&
                ...
                (pdat = set_data(co, DRD_LQ_PLR_DATA, sizeof(struct lq_plr_data)))) pdat->mark[dat->hurt_markID] = 1;
        }
