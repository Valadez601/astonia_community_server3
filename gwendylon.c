    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 0:
                    if ((ch[co].flags & CF_PAID) && ch[co].exp == 0 && !(ch[co].flags & CF_HARDCORE)) quiet_say(cn, "\260c3Hello, %s. Dost thou wish to become a \260c4Hardcore\260c3 character?", ch[co].name);
...
int james_raisehint(int cn, int doraise) {
    ...
    if (ch[cn].value[1][V_FLASH] > 1 && ch[cn].value[1][V_FLASH] > ch[cn].value[1][V_FIREBALL]) {
        fire = 0;
        flash = 1;
    }
    if (ch[cn].value[1][V_FIREBALL] > 1 && ch[cn].value[1][V_FIREBALL] > ch[cn].value[1][V_FLASH]) {
        fire = 1;
        flash = 0;
    }
    ...
    if (misc) {
        for (v = 0; v < V_MAX; v++) {
            ...
            if (!raise[v] && can_raise(cn, v)) raise[v] = 1.0 / (raise_cost(v, ch[cn].value[1][v], (ch[cn].flags & (CF_MAGE | CF_WARRIOR)) == (CF_MAGE | CF_WARRIOR)) * weight);
        }
    }
