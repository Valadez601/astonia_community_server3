...
static int enhance_cannot_use(int in, int cn) {
    ...
    for (m = 0; m < MAXMOD; m++) {
        if (it[in].mod_value[m]) {
            switch (it[in].mod_index[m]) {
            case -V_ARMORSKILL:
                if (ch[cn].value[1][V_ARMORSKILL] < it[in].mod_value[m] + 10) return 1;
                else break;
            case -V_DAGGER:
                if (ch[cn].value[1][V_DAGGER] < it[in].mod_value[m] + 10) return 1;
                else break;
            case -V_STAFF:
                if (ch[cn].value[1][V_STAFF] < it[in].mod_value[m] + 10) return 1;
                else break;
            case -V_SWORD:
                if (ch[cn].value[1][V_SWORD] < it[in].mod_value[m] + 10) return 1;
                else break;
            case -V_TWOHAND:
                if (ch[cn].value[1][V_TWOHAND] < it[in].mod_value[m] + 10) return 1;
                else break;
            }
        }
    }
    ...
    for (m = 0; m < MAXMOD; m++) {
        if (it[in2].mod_value[m]) {
            ...
            case V_WEAPON:
                ...
                break;
            default:
                if (it[in2].mod_index[m] >= 0 && it[in2].mod_value[m] < 20) it[in2].mod_value[m]++;
                break;
            }
        }
    }
