...
void close_area(int n) {
    ...
    for (m = 0; m < area[n].max_spawn; m++) {
        in = area[n].spawn[m];
        if (*(unsigned int *)(it[in].drdata + 0) && *(unsigned int *)(it[in].drdata + 0) != 0xfffff000) remove_party(*(unsigned int *)(it[in].drdata + 0), NULL);
    }
}
...
void spawner(int in, int cn) {
    ...
    if (!spawner_sub(in, in2, ch[cn].ID, ch[cn].name, ppd)) log_char(cn, LOG_SYSTEM, 0, "No space to drop char or max worker reached.");
}
        ...
        if (msg->type == NT_CHAR) {
            ...
            if ((ch[co].flags & CF_PLAYER) && char_dist(cn, co) < 16 && char_see_char(cn, co) && ch[cn].driver != CDR_LOSTCON &&
                ...
                case 0:
                    ...
                    break;
                case 1:
                    if (get_army_rank_int(co) >= 8) ppd->boss_stage++;
                    else break;
