    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_CHAR) {
            ...
            if (ppd) {
                ...
                case 10:
                    if ((in = ch[co].item[WN_RHAND]) && it[in].ID == IID_HARDKILL && it[in].drdata[37] < 36) say(cn, "So that is how one can kill them. Thou wilt need to find all three stone circles and perform the ritual in each one, then, %s.", ch[co].name);
