...
int build_warrior(int x, int y, int level) {
    ...
    for (n = 0; n < V_MAX; n++) {
        ...
        case V_RAGE:
            if (ch[cn].flags & CF_ARCH) val = max(1, base - 20);
            else val = 0;
            break;
...
void build_door(int x, int y, int keyid, int keys) {
    ...
    if (keys > 0) *(unsigned long *)(it[in].drdata + 0) = MAKE_ITEMID(DEV_ID_MAZE1, keyid);
    else *(unsigned long *)(it[in].drdata + 0) = 0;
    if (keys > 1) *(unsigned long *)(it[in].drdata + 4) = MAKE_ITEMID(DEV_ID_MAZE2, keyid);
    else *(unsigned long *)(it[in].drdata + 4) = 0;
...
int create_maze(int base, int level, int do_fake, int keys, const int *warrior, const int *mage, const int *seyan, int teleport, int cnr, int show) {
    ...
    while (m > 0) {
        ...
        if (!cell[n].l && n % xsize > 0 && cell[n - 1].special == m - 1) {
            ...
            n = n - 1;
        } else if (!cell[n].t && n / xsize > 0 && cell[n - xsize].special == m - 1) {
            if (m == tmp) {
                fake = n;
                fakedir = 2;
            }
            n = n - xsize;
        } else if (n % xsize < xsize - 1 && !cell[n + 1].l && cell[n + 1].special == m - 1) {
            n = n + 1;
            if (m == tmp) {
                fake = n;
                fakedir = 1;
            }
        } else if (n / xsize < ysize - 1 && !cell[n + xsize].t && cell[n + xsize].special == m - 1) {
            n = n + xsize;
            if (m == tmp) {
                fake = n;
                fakedir = 2;
...
void dungeonmaster(int cn, int ret, int lastact) {
    ...
    for (msg = ch[cn].msg; msg; msg = next) {
        ...
        if (msg->type == NT_TEXT) {
            ...
            case 2:
                say(cn, "Use: 'attack <nr>' to attack clan <nr>, 'enter <nr>' to enter catacomb <nr> or 'list' to get a listing of all catacombs.");
                break;
            case 3:
                list_dungeon(cn, dat);
                break;
...
void dungeonfighter(int cn, int ret, int lastact) {
    ...
    if ((ch[cn].hp < ch[cn].value[0][V_HP] * POWERSCALE / 2 || ch[cn].mana < ch[cn].value[0][V_MANA] * POWERSCALE / 2) && dat->simple_pots_taken < 5 && !flag) {
        if (dat->damage_done > 10 && dat->damage_done > dat->damage_taken / 16) {
            need = max(ch[cn].value[0][V_HP] - ch[cn].hp / POWERSCALE, ch[cn].value[0][V_MANA] - ch[cn].mana / POWERSCALE);
            add = nr = 0;
